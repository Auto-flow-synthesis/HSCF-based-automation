"""
Automated synthesis script for Brexpiprazole.

This script controls the hardware sequence for the multistep automated synthesis of Brexpiprazole through a Tkinter-based GUI.

Main sections
-------------
1. Device imports and object initialization
2. Hardware initialization (autoprog_init)
3. Automated synthesis workflow execution (autoprog_start)
4. Safe shutdown of hardware (autoprog_abort)
5. GUI labels describing each programmed action
6. Timeline values corresponding to the programmed sequence
"""
...
def autoprog_start():
	"""
    Execute the full automated multistep synthesis workflow for Brexpiprazole.

    Workflow structure
    ------------------
    1. Valve initialization
    2. First-step preparation, reaction, precipitation, and transfer
    3. Second-step reaction, rotary evaporation, quench, precipitation, and transfer
    4. Third-step preparation, reaction, and precipitation
    5. Automatic shutdown at the end of the sequence
    """
	global valve_1
	global valve_2
	global valve_3
	...
	# 'D' : pump channel identifier
	# 0.00 : execution delay (min)
	# 25000.00 : flow rate in µL/min
	# lbl_15 : GUI label for displaying action status
	milliGAT_D.milliGAT_stoping('D', 6.00, lbl_16)
	

	# first_step_reaction; first_step_wash_tubing
	watson_1.watson_pumping('RL', 0.00, 1985.0, lbl_17)
	# 'RL' : rotation direction
	# 0.00 : execution delay (min)
	# 1985.0 : rotation speed (RPM)
	# lbl_17 : GUI label for displaying action status
	...
	# Set rotation speed of Heidolph
	# 0.00       : execution delay (min)
	# 800.000000 : rotation speed (rpm)
	# lbl_125    : GUI label for displaying action status
	heidolph_1.heidolph_heating(0.00, 80.000000, lbl_126)
	# Set heating temperature of Heidolph
	# 0.00       : execution delay (min)
	# 80.000000  : target temperature (°C)
	# lbl_126    : GUI label for displaying action status
