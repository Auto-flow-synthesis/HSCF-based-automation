"""
Automated synthesis script for Benzoxazinone derivatives.

This script controls the hardware sequence for the multistep automated synthesis of Esonarimod through a Tkinter-based GUI.

Main sections
-------------
1. Device imports and object initialization
2. Hardware initialization (autoprog_init)
3. Definition of unit operations
4. Automated synthesis workflow execution (autoprog_start)
5. Safe shutdown of hardware (autoprog_abort)
6. GUI labels describing each programmed action
7. Timeline values corresponding to the programmed sequence
"""
...
def autoprog_init():
	"""
    Initialize all hardware modules and establish communication.

    Devices initialized in this script include:
    - 19 selector valves
	- 1 relay
    - 3 Watson-Marlow pump
    - 6 MilliGAT pump
	- 1 oushisheng pump
	- 1 sf10 pump
    - 1 rotary evaporator
	...
	# 1: valve device address
	# frame.lbl_init: GUI label for status feedback
	global valve_2
...
def initialize_valves():
	...
	# 0.005: execution delay (min)
	# "1": target valve position
	# lbl_0: GUI label for displaying action status
...
def initialize_milliGAT():
	...
	# "S": pump channel identifier
	# 0.005: execution delay (min)
	# 0.00: flow rate (µL/min)
	# lbl_1: GUI label for displaying action status
...
def rotaVao_10min_200to75mbar():   
	...
	# Control overall status of rotary evaporator
	# True   : turn ON (start operation)
	# 0.00   : execution delay (s)
	# lbl_2 : GUI label for displaying action status
	rotaVap_1.lift_height(220, lbl_2)
	# Adjust lift height of rotary evaporator
	# 220   : lift height (mm)
	# lbl_2: GUI label for displaying action status
	rotaVap_1.heating(50, True, lbl_2)
	# Set heating bath temperature of rotary evaporator
	# 50    : target temperature (°C)
	# True  : enable heating (ON)
	# lbl_2: GUI label for displaying action status
	rotaVap_1.rotation_speed(50, True, 0.00,  lbl_2)
	# Set rotation speed of rotary evaporator
	# 50    : rotation speed (rpm)
	# True  : enable rotation (ON)
	# 0.00  : execution delay (s)
	# lbl_2: GUI label for displaying action status
	rotaVap_1.vacuum_P(75, True, 0.00,  lbl_2)
	# Set vacuum pressure of rotary evaporator
	# 75   : target vacuum pressure (mbar)
	# True  : enable vacuum (ON)
	# 0.00  : execution delay (s)
	# lbl_2: GUI label for displaying action status
...
def rotaVap_colletion():   
	...
	# 0.00: execution delay (min)
	# 1000.0: rotation speed (RPM)
	# lbl_4: GUI label for displaying action status
...
def rotaVap_wash_Acetone3():   
	...
	valve_8.valve_go(1.00, "1", lbl_5)
	valve_9.valve_go(0.00, "1", lbl_5)
	# exclude the collected condensate
...
def fourth_step_reaction():
	...
	# "1": pump channel identifier
	# 0.00: execution delay (min)
	# 100.0: flow rate (mL/min)
	# lbl_29: GUI label for displaying action status
...
def reaction_1():
	"""
    Execute reaction 1 workflow.
    This function performs the full sequence of operations for the 1st reaction step
    """
	first_step_preparation() # Prepare reagents and system
	first_step_reaction()  # Perform reaction step 1
	first_step_wash_tubing()  # Clean tubing to avoid cross-contamination
	first_step_filtration() # Filtration to move solid byproduct
	first_step_wash_filtration() # Wash the filtration unit to remove residual product
	first_step_final_cleaning() # Perform final cleaning of the system to prepare for the next step 
def reaction_2():
	"""
    Execute reaction 2 workflow.
    This function performs the full sequence of operations for the 2nd reaction step
    """
	second_step_preparation()  # Prepare reagents and system
	second_step_reaction()  # Perform reaction step 2
	second_step_wash_tubing()   # Clean tubing to avoid cross-contamination
	second_step_filtration() # Filtration to move solid byproduct
	second_step_wash_filtration()  # Wash the filtration unit to remove residual product
	rotaVao_20min_200to75mbar() # Conduct rotary evaporation for 20 min with pressure reduced from 200 to 75 mbar
	rotaVap_colletion() # Collect concentrated product after solvent removal
	second_step_recrystallization() # Purify product via recrystallization
	second_step_final_cleaning() # Perform final cleaning of modules to reset system for subsequent operations
	rotaVap_wash_Acetone3() # Clean rotary evaporator using acetone (3 washing cycles)
	second_step_dissolve_product() # Redissolve purified product for transfer
def reaction_3():
	"""
    Execute reaction 3 workflow.
    This function performs the full sequence of operations for the 3rd reaction step
    """
	third_step_reaction_and_separation() # Execute step 3 reaction followed by liquid/liquid separation
	rotaVao_20min_200to75mbar() # Conduct rotary evaporation for 20 min with pressure reduced from 200 to 75 mbar
	rotaVap_colletion()  # Collect concentrated product after solvent removal
	third_step_transfer_for_FC() # Transfer crude product to flash chromatography (FC) module
	rotaVap_wash_Acetone1() # Clean rotary evaporator using acetone (1 washing cycle)
	third_step_FC_purge() # Purge FC system to  remove residual solvent (Toluene)
	third_step_FC() # Perform flash chromatography purification
	rotaVao_10min_200to75mbar()  # Conduct rotary evaporation for 10 min with pressure reduced from 200 to 75 mbar
	rotaVap_move_waste() # Remove collected condensate (solvent) from the rotary evaporator waste container
	third_step_FC() 
...
def reaction_4():
	"""
    Execute reaction 4 workflow.
    This function performs the full sequence of operations for the 4th reaction step
    """
	fourth_step_preparation()  # Prepare reagents and system
	fourth_step_reaction()  # Perform reaction step 1
def reaction_1_to_3():
	"""
    Execute reaction 1,2,3 workflow in linear sequence.
    """
	reaction_1()
	reaction_2()
	reaction_3()
def reaction_4_and_1_to_3():
	"""
	Parallelized execution of Reactions 1, 2, 3 with Reaction 4
    """
...
def autoprog_start():
	"""
	Execute the semi-continuous automated multistep synthesis workflow for Benzoxazinone derivatives
    """
	global valve_1
	global valve_2
	global valve_3
...
def autoprog_abort():
	"""
    Safely stop the workflow and close communication with all devices.
    """
	valve_1.close()
	valve_2.close()
	valve_3.close()
