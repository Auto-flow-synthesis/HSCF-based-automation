...
def topWin(frame):  #hide all the window, when use grid() method, display the selected window
    ...
    fr_rotavap.grid_remove()
    fr_relay.grid_remove()
...
def savefile():
    ...
    with open(filepath, "w+") as file:
        ...
        cmd_str = ''.join(txt_output_cmd)  #connect all element in list txt_output_cmd to one single variable named cmd_str
        total_init = cmd_str.count('global')  #count how many times 'global' appears in the string
...
def instru_init(instru, num, COM):
    ...
    if instru == "asiapump":
        ...
        # declare global variable asiapump_n,define status and initialize it
        ...
        txt_output_para.append("%s_%s=%sFn()\n" %(instru, num, instru))
        txt_output_init.append("\tglobal %s_%s\n" %(instru, num) +
                               "\t%s_%s.%s_init('%s', frame.lbl_init)\n" %(instru, num, instru, num) +
                               "\t%s_%s.lbl_%s_sta = lbl_%s_%s_sta\n" %(instru, num, instru, instru, num))  
    else:
        txt_output_para.append("%s_%s=%sFn()\n" %(instru,num,instru))
        txt_output_init.append("\tglobal %s_%s\n" %(instru, num) +
...
def asiapump_pumping(num, chan, timeEntry, rateEntry):  
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Asiapump %d channel %d pumps at %d ul/min, total time usage %.2f" %(time, num, (chan+1), rate, sum(timeline)))
        ...
        txt_output_interface.append("After %.2f min, Asiapump %d channel %d pumps at %d ul/min, total time usage %.2f"%(time, num, (chan+1), rate, sum(timeline)))
        txt_output_interface.append("After 0 min, Asiapump %d channel %d pumps at %d ul/min"%(num, (chan+1), rate))
   
        #write the command to file

        txt_output_cmd.append("\tasiapump_%d.asiapump_pumping(%d, %.2f, %d," %(num, chan, time, rate))
...
def asiapump_filling(num, chan, timeEntry, rateEntry):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Asiapump %d channel %d fills at %d ul/min, total time usage %.2f" %(time, num, (chan+1), rate, sum(timeline)))
        fr_cmdInt.insert(tk.END, "After 0 min, Asiapump %d channel %d fills at %d ul/min" %(num, (chan+1), rate))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Asiapump %d channel %d fills at %d ul/min, total time usage %.2f"%(time, num, (chan+1), rate, sum(timeline)))
        txt_output_interface.append("After 0 min, Asiapump %d channel %d fills at %d ul/min"%(num, (chan+1), rate))

        txt_output_cmd.append("\tasiapump_%d.asiapump_filling(%d, %.2f, %d,"%(num, chan, time, rate))
...
def asiapump_emptying(num, chan, timeEntry, rateEntry):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Asiapump %d channel %d empties at %d ul/min, total time usage %.2f" %(time, num, (chan+1), rate, sum(timeline)))
        fr_cmdInt.insert(tk.END, "After 0 min, Asiapump %d channel %d empties at %d ul/min" %(num, (chan+1), rate))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Asiapump %d channel %d empties at %d ul/min, total time usage %.2f"%(time, num, (chan+1), rate, sum(timeline)))
        txt_output_interface.append("After 0 min, Asiapump %d channel %d empties at %d ul/min"%(num, (chan+1), rate))

        txt_output_cmd.append("\tasiapump_%d.asiapump_emptying(%d, %.2f, %d,"%(num, chan, time, rate))
...
def asiapump_stoping(num, chan, timeEntry):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Asiapump %d channel %d stops, total time usage %.2f" %(time, num, (chan+1), sum(timeline)))
        fr_cmdInt.insert(tk.END, "After 0 min, Asiapump %d channel %d stops" %(num, (chan+1)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Asiapump %d channel %d stops, total time usage %.2f"%(time, num, (chan+1), sum(timeline)))
        txt_output_interface.append("After 0 min, Asiapump %d channel %d stops"%(num, (chan+1)))

        txt_output_cmd.append("\tasiapump_%d.asiapump_stoping(%d, %.2f,"%(num, chan, time))
...
def sf10_pumping(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, SF10 %d valve %s pumps at %.1fml/min, total time usage %.2f" %(time, num, chan, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, SF10 %d valve %s pumps at %.1fml/min, total time usage %.2f"%(time, num, chan, rate, sum(timeline)))

        txt_output_cmd.append("\tsf10_%d.sf10_pumping('%s', %d.2f, %.1f,"%(num, chan, time, rate))
...
def sf10_gaspumping(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, SF10 %d valve %s gaspumps at %.1fml/min, total time usage %.2f" %(time, num, chan, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, SF10 %d valve %s gaspumps at %.1fml/min, total time usage %.2f"%(time, num, chan, rate, sum(timeline)))

        txt_output_cmd.append("\tsf10_%d.sf10_gaspumping('%s', %.2f, %.1f,"%(num, chan, time, rate))
...
def sf10_regpresing(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, SF10 %d valve %s sets gas regulator pressure at %1.f bar, total time usage %.2f" %(time, num, chan, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, SF10 %d valve %s sets gas regulator pressure at %.1f bar, total time usage %.2f"%(time, num, chan, rate, sum(timeline)))

        txt_output_cmd.append("\tsf10_%d.sf10_regpresing('%s', %.2f, %.1f,"%(num, chan, time, rate))
...
def sf10_setramping(num, chan, timeEntry, rateEntry_1, rateEntry_2, periodEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, SF10 %d valve %s sets ramp speed from %.1f ml/min to %.1f ml/min over %.1f min, total time usage %.2f" %(time, num, chan, rate_1, rate_2, period, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, SF10 %d valve %s sets ramp speed from %.1f ml/min to %.1f ml/min over %.1f min, total time usage %.2f"%(time, num, chan, rate_1, rate_2, period, sum(timeline)))

        txt_output_cmd.append("\tsf10_%d.sf10_setramping('%s', %.2f, %.1f, %.1f, %.1f,"%(num, chan, time, rate_1, rate_2, period))
...
def sf10_stoping(num, chan, timeEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, SF10 %d valve %s stops, total time usage %.2f" %(time, num, chan, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, SF10 %d valve %s stops, total time usage %.2f"%(time, num, chan, sum(timeline)))

        txt_output_cmd.append("\tsf10_%d.sf10_stoping('%s', %.2f,"%(num, chan, time))
...
def milliGAT_pumping(num, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, MilliGAT %s pumps at %d ul/min, total time usage %.2f" %(time, num, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, MilliGAT %s pumps at %d ul/min, total time usage %.2f min"%(time, num, rate, sum(timeline)))

        txt_output_cmd.append("\tmilliGAT_%s.milliGAT_pumping('%s', %.2f, %.2f,"%(num, num, time, rate))
...
def milliGAT_volumning(num, timeEntry, volumnEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, MilliGAT %s pumps %d ul at %d ul/min, total time usage %.2f" %(time, num, volumn, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, MilliGAT %s pumps %d ul at %d ul/min, total time usage %.2f"%(time, num, volumn, rate, sum(timeline)))

        txt_output_cmd.append("\tmilliGAT_%s.milliGAT_volumning('%s', %.2f, %d, %d,"%(num, num, time, volumn, rate))
...
def milliGAT_stoping(num, timeEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, MilliGAT %s stops, total time usage %.2f" %(time, num, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, MilliGAT %s stops, total time usage %.2f"%(time, num, sum(timeline)))

        txt_output_cmd.append("\tmilliGAT_%s.milliGAT_stoping('%s', %.2f,"%(num, num, time))
...
def valve_going(num, timeEntry, position):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Valve %d goes to position %s, total time usage %.2f" %(time, num, position, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Valve %d goes to position %s, total time usage %.2f"%(time, num, position, sum(timeline)))

        if type(position) == int:
            txt_output_cmd.append("\tvalve_%d.valve_go(%.2f, %s,"%(num, time, position))
        else:
            position = "\"" + position + "\""
            txt_output_cmd.append("\tvalve_%d.valve_go(%.2f, %s,"%(num, time, position))
...
def oushisheng_pumping(num, timeEntry, rateEntry, pumptype):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Ou Shisheng %d pumps at %d ul/min, total time usage %.2f" %(time, num, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Ou Shisheng %d pumps at %d ul/min, total time usage %.2f"%(time, num, rate, sum(timeline)))

        txt_output_cmd.append("\toushisheng_%d.oushisheng_pumping(%.2f, %d,"%(num, time, rate))
...
def oushisheng_stoping(num, timeEntry):
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Ou Shisheng %d stops, total time usage %.2f" %(time, num, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("'After %.2f min, Ou Shisheng %d stops, total time usage %.2f"%(time, num, sum(timeline)))

        txt_output_cmd.append("\toushisheng_%d.oushisheng_stoping(%.2f,"%(num, time))
...
def heidolph_rotating(num, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Heidolph %d rotates at %d rpm, total time usage %.2f" %(time, num, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Heidolph %d rotates at %d rpm, total time usage %.2f"%(time, num, rate, sum(timeline)))

        txt_output_cmd.append("\theidolph_%d.heidolph_rotating(%.2f, %f,"%(num, time, rate))
...
def heidolph_heating(num, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Heidolph %d heats to %d degree Celsius, total time usage %.2f" %(time, num, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Heidolph %d heats to %d degree Celsius, total time usage %.2f"%(time, num, rate, sum(timeline)))

        txt_output_cmd.append("\theidolph_%d.heidolph_heating(%.2f, %f,"%(num, time, rate))
...
def heidolph_stop_rotating(num, timeEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Heidolph %d stops rotating, total time usage %.2f" %(time, num, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Heidolph %d stops rotating, total time usage %.2f"%(time, num, sum(timeline)))

        txt_output_cmd.append("\theidolph_%d.heidolph_stop_rotating(%.2f,"%(num, time))
...
def heidolph_stop_heating(num, timeEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Heidolph %d stops heating, total time usage %.2f" %(time, num, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Heidolph %d stops heating, total time usage %.2f"%(time, num, sum(timeline)))

        txt_output_cmd.append("\theidolph_%d.heidolph_stop_heating(%.2f,"%(num, time))
...
def watson_pumping(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Watson Marlow %d direction %s pumps at %.1fRPM, total time usage %.2f" %(time, num, chan, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Watson Marlow %d direction %s pumps at %.1f RPM, total time usage %.2f" %(time, num, chan, rate, sum(timeline)))

        txt_output_cmd.append("\twatson_%d.watson_pumping('%s', %.2f, %.1f,"%(num, chan, time, rate))
...
def watson_dosing(num, chan, timeEntry, rateEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Watson Marlow %d direction %s doses %.1f tacho, total time usage %.2f" %(time, num, chan, rate, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Watson Marlow %d direction %s doses %.1f tacho, total time usage %.2f"%(time, num, chan, rate, sum(timeline)))

        txt_output_cmd.append("\twatson_%d.watson_dosing('%s', %.2f, %.1f,"%(num, chan, time, rate))
...
def watson_stoping(num, chan, timeEntry): #Firstly, check the parametere. Secondly, write the command to file. Thirdly, update the interface.
    ...
    try:#check parameters
        ...
        fr_cmdInt.insert(tk.END, "After %.2f min, Watson Marlow %d direction %s stops, total time usage %.2f" %(time, num, chan, sum(timeline)))
        timeline.append(time)

        txt_output_interface.append("After %.2f min, Watson Marlow %d direction %s stops, total time usage %.2f"%(time, num, chan, sum(timeline)))

        txt_output_cmd.append("\twatson_%d.watson_stoping('%s', %.2f,"%(num, chan, time))
...
#for rotaVap
def rotaVap_rot(rpm, timeEntry):

    try:
        time = float(timeEntry.get())
        rate = int(float(rpm.get()))
        
        fr_cmdInt.insert(tk.END, "After %.2f min, rotary vaporator start to rotate at %d rpm, total time usage %.2f" %(time, rate, sum(timeline)))
        timeline.append(time)
        txt_output_interface.append("After %.2f min, rotary vaporator start to rotate at %d rpm, total time usage %.2f" %(time, rate, sum(timeline)))
        # txt_output_cmd.append("\ttime.sleep(%.2f)" %(time*60))
        txt_output_cmd.append("\trotaVap_1.rotation_speed(%d, True, %.2f, " %(rate, time*60))  

    except:
        tk.messagebox.showinfo("Warning", traceback.format_exc())


def rotaVap_vac(pressure, timeEntry):
    try:
        time = float(timeEntry.get())
        pre = int(float(pressure.get()))
        fr_cmdInt.insert(tk.END, "Rotary vaporator start to vacuum at %d mbar, continue for %.2f min,  total time usage %.2f" %(pre, time, sum(timeline)))
        timeline.append(time)
        txt_output_interface.append("Rotary vaporator start to vacuum at %d mbar, stop after %.2f min, total time usage %.2f"  %(pre, time, sum(timeline)))
        
        txt_output_cmd.append("\trotaVap_1.vacuum_P(%d, True, %.2f, "  %(pre, time*60))
        # txt_output_cmd.append("\ttime.sleep(%.2f)" %(time*60))  
...
def rotaVap_stopVac():
    try:
        ...
        fr_cmdInt.insert(tk.END, "Rotary vaporator stop vacuum, total time usage %.2f" %(sum(timeline)))
        
        txt_output_interface.append("Rotary vaporator stop vacuum, total time usage %.2f" %(sum(timeline)))
 
        txt_output_cmd.append("\trotaVap_1.vacuum_P(1300, False, 0," )
    except:
        tk.messagebox.showinfo("Warning", traceback.format_exc())
...
def rotaVap_stopRot():
    try:
        ...
        fr_cmdInt.insert(tk.END, "Rotary vaporator stop rotate, total time usage %.2f" %(sum(timeline)))
        
        txt_output_interface.append("Rotary vaporator stop rotate total time usage %.2f" %(sum(timeline)))

        
        txt_output_cmd.append("\trotaVap_1.rotation_speed(0, False, 0," )
    except:
        tk.messagebox.showinfo("Warning", traceback.format_exc())

def rotaVap_lift(height):
    try:
        lift_high = int(float(height.get()))

        fr_cmdInt.insert(tk.END, "Lift flask for %d mm" %(lift_high))
        
        txt_output_interface.append("Lift flask for %d mm" %(lift_high))
 
        txt_output_cmd.append("\trotaVap_1.lift_height(%d," %(lift_high))
    except:
        tk.messagebox.showinfo("Warning", traceback.format_exc())  
 

def rotaVap_Global(status, timeEntry):
    try:
        time = float(timeEntry.get())
        if status == True:
            fr_cmdInt.insert(tk.END, "After %.2f min, Globalstatus start  total time usage %.2f" %(time, sum(timeline)))
            txt_output_interface.append("After %.2f min, Globalstatus start, total time usage %.2f"  %(time, sum(timeline)))
        else:
            fr_cmdInt.insert(tk.END, "After %.2f min, Globalstatus stop, total time usage %.2f" %(time, sum(timeline)))
            txt_output_interface.append("After %.2f min, Globalstatus start, total time usage %.2f"  %(time, sum(timeline)))
        timeline.append(time)
        
        txt_output_cmd.append("\trotaVap_1.Global_status(%s, %.2f,"  %(status, time*60))
        # txt_output_cmd.append("\ttime.sleep(%.2f)" %(time*60))  

    except:
        tk.messagebox.showinfo("Warning", traceback.format_exc())

def rotaVap_heating(temp):
    try:
        temperature = int(float(temp.get()))

        fr_cmdInt.insert(tk.END, "adjust temperature to %d " %(temperature))
        
        txt_output_interface.append("adjust temperature to %d " %(temperature))
 
        txt_output_cmd.append("\trotaVap_1.heating(%d, True," %(temperature))
    except:
        tk.messagebox.showinfo("Warning", traceback.format_exc())  

# for relay
def relay_going(num, timeEntry, position):
    ...
    fr_cmdInt.insert(tk.END, "After %.2f min, Relay %d goes to position %s, total time usage %.2f" %(time, num, position, sum(timeline)))
    timeline.append(time)

    txt_output_interface.append("After %.2f min, Relay %d goes to position %s, total time usage %.2f"%(time, num, position, sum(timeline)))

    if type(position) == int:
        txt_output_cmd.append("\trelay_%d.relay_go(%.2f, %d, %s,"%(num, time, num, position))
    else:
        position = "\"" + position + "\""
        txt_output_cmd.append("\trelay_%d.relay_go(%.2f, %d, %s,"%(num, time, num, position))
...
for i in range(0, 3):
    for j in range(0, 2):
        milliGAT_flag += 1
        tk.Label(fr_milliGAT_init, text="MilliGAT %d" % milliGAT_flag, width=8, height=2).grid(row=i, column=4 * j,
                                                                                               padx=2, pady=3)
        milliGAT_entry = tk.Entry(fr_milliGAT_init, width=3)
        milliGAT_entry.grid(row=i, column=4 * j + 1, padx=2, pady=3)
        milliGAT_num[milliGAT_flag - 1] = milliGAT_entry
        globals()["com_milliGAT_%d_init" % milliGAT_flag] = ttk.Combobox(fr_milliGAT_init, width=8, value=com_value)
        globals()["com_milliGAT_%d_init" % milliGAT_flag].grid(row=i, column=4 * j + 2, padx=2, pady=3)


        def instru_init_wrapper(flag):
            instru_init("milliGAT", str(milliGAT_num[flag - 1].get()),
                        globals()["com_milliGAT_%d_init" % flag].get())


        tk.Button(fr_milliGAT_init, text="Add", width=4, height=1,
                  command=lambda f=milliGAT_flag: instru_init_wrapper(f)).grid(row=i, column=4 * j + 3, padx=2, pady=3)
...
com_milliGAT_num = tk.Entry(fr_milliGAT_num, width = 4)
...
fr_rotavap = tk.Frame(window, width =450, height = 700,relief = tk.FLAT, bd =2)
fr_rotavap.grid(row=0, column=1, padx=5,pady=5)
fr_rotavap.grid_propagate(0)
fr_rotavap.rowconfigure(4, weight=1)
fr_rotavap.columnconfigure(0, weight=1)
   

#Rotavap Initialization

tk.Label(fr_rotavap, text = "BUCHI Rotavap Initialization Command Editor", width=50, height=2, font=("calibri",12,"bold")).grid(row=0, column=0, padx=3, pady=3)

fr_rotavap_init = tk.Frame(fr_rotavap, width=65, height = 185, relief=tk.GROOVE, bd=2)
fr_rotavap_init.grid(row=1, column=0, padx=3, pady=3, sticky="nsew")



tk.Label(fr_rotavap_init, text = "Rotavap", width=10, height=2).grid(row=1, column=1, padx=3, pady=3)    

        # globals()["com_rotavap_%d_init"%(i+1+j)] = ttk.Combobox(fr_rotavap_init, width=8, value = com_value)
       
        # globals()["com_rotavap_%d_init"%(i+1+j)].grid(row=i, column=j+1, padx=3, pady=3)
       
        # tk.Button(fr_rotavap_init, text = "Add", width=5, height=1, command = lambda:rotavap_init((i+1+j),globals()["com_rotavap_%d_init"%(i+1+j)].get())).grid(row=i, column=j+2, padx=3, pady=3)

tk.Button(fr_rotavap_init, text = "Add", width=5, height=1, command = lambda:instru_init("rotaVap",1,1)).grid(row=1, column=2, padx=3, pady=3)


#RotaVap number
       
fr_rotavap_num = tk.Frame(fr_rotavap, width=65, height = 100, relief=tk.FLAT, bd=2)
fr_rotavap_num.grid(row=2, column=0, padx=3, pady=3, sticky="nsew")

tk.Label(fr_rotavap_num, text = "\tCommand Editor for BUCHI RotaVap", width=40, height=2, font=("calibri",12,"bold")).grid(row=0, column=0, padx=3, pady=3)

com_rotavap = [1]

com_rotavap_num = ttk.Combobox(fr_rotavap_num, width=4, height=6, font=("calibri",12,"bold"), value = com_rotavap)
com_rotavap_num.grid(row=0, column=1, padx=3, pady=3)

#Rotavap Function
       
fr_rotavap_fn = tk.Frame(fr_rotavap, width=65, height = 380, relief=tk.GROOVE, bd=2)
fr_rotavap_fn.grid(row=3, column=0, padx=3, pady=3, sticky="nsew")
fr_rotavap_fn.rowconfigure(8, minsize=10, weight=1)
fr_rotavap_fn.columnconfigure(7, minsize=10, weight=1)
fr_rotavap_fn.grid_propagate(0)

# tk.Label(fr_rotavap_fn, text = "Actual", width=10, height=1).grid(row=0, column=1)
# tk.Label(fr_rotavap_fn, text = "Set", width=10, height=1).grid(row=0, column=4)

tk.Label(fr_rotavap_fn, text = "Start rotat ", width=12, height=2).grid(row=1, column=0, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "Vacuum", width=12, height=2).grid(row=2, column=0, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "Stop rotat", width=12, height=2).grid(row=3, column=0, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "Stop vac", width=12, height=2).grid(row=4, column=0, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "Lift bulb", width=12, height=2).grid(row=5, column=0, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "Global Status", width=12, height=2).grid(row=6, column=0, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "Heating", width=12, height=2).grid(row=7, column=0, padx=3,pady=3)
...
#     globals()["ent_rotavap_" + str(i) + "1"] = tk.StringVar()
#     globals()["ent_rotavap_" + str(i) + "1"].set(0)
       
#     tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_" + str(i) + "1"], width=8).grid(row=i, column=2, padx=3, pady=3)


tk.Label(fr_rotavap_fn, text = "rpm ", width=8, height=2).grid(row=1, column=3, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "mbar", width=8, height=2).grid(row=2, column=3, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = " ", width=8, height=2).grid(row=3, column=3, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = " ", width=8, height=2).grid(row=4, column=3, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "mm", width=8, height=2).grid(row=5, column=3, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "min", width=8, height=2).grid(row=6, column=3, padx=3,pady=3)
tk.Label(fr_rotavap_fn, text = "℃", width=8, height=2).grid(row=7, column=3, padx=3,pady=3)

globals()["ent_rotavap_rpm"] = tk.StringVar()
globals()["ent_rotavap_rpm"].set(0)
       
tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_rpm"], width=5).grid(row=1, column=2, padx=3, pady=3)

globals()["ent_rotavap_mbar"] = tk.StringVar()
globals()["ent_rotavap_mbar"].set(0)
       
tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_mbar"], width=5).grid(row=2, column=2, padx=3, pady=3)

globals()["ent_rotavap_mm"] = tk.StringVar()
globals()["ent_rotavap_mm"].set(0)
       
tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_mm"], width=5).grid(row=5, column=2, padx=3, pady=3)

globals()["ent_rotavap_rtime"] = tk.StringVar()
globals()["ent_rotavap_rtime"].set(0)
       
tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_rtime"], width=5).grid(row=1, column=5, padx=3, pady=3)

globals()["ent_rotavap_vtime"] = tk.StringVar()
globals()["ent_rotavap_vtime"].set(0)
       
tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_vtime"], width=5).grid(row=2, column=5, padx=3, pady=3)

globals()["ent_rotavap_gtime"] = tk.StringVar()
globals()["ent_rotavap_gtime"].set(0)

tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_gtime"], width=5).grid(row=6, column=2, padx=3, pady=3)

globals()["ent_rotavap_temp"] = tk.StringVar()
globals()["ent_rotavap_temp"].set(0)

tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_rotavap_temp"], width=5).grid(row=7, column=2, padx=3, pady=3)

globals()["ent_global_status"] = tk.StringVar()
globals()["ent_global_status"].set(0)
       
tk.Entry(fr_rotavap_fn, textvariable = globals()["ent_global_status"], width=5).grid(row=8, column=2, padx=3, pady=3)


tk.Label(fr_rotavap_fn, text = "min", width=5, height=2).grid(row=1, column=7, padx=3, pady=3)
tk.Label(fr_rotavap_fn, text = "min", width=5, height=2).grid(row=2, column=7, padx=3, pady=3)
tk.Label(fr_rotavap_fn, text = " ", width=5, height=2).grid(row=3, column=7, padx=3, pady=3)
tk.Label(fr_rotavap_fn, text = " ", width=5, height=2).grid(row=4, column=7, padx=3, pady=3)
tk.Label(fr_rotavap_fn, text = "  ", width=5, height=2).grid(row=5, column=7, padx=3, pady=3)
...
tk.Button(fr_rotavap_fn, text = "Add", width=5, height=1, command = lambda:rotaVap_rot(ent_rotavap_rpm, ent_rotavap_rtime)).grid(row=1, column=8, padx=3, pady=3)
tk.Button(fr_rotavap_fn, text = "Add", width=5, height=1, command = lambda:rotaVap_vac(ent_rotavap_mbar, ent_rotavap_vtime)).grid(row=2, column=8, padx=3, pady=3)
tk.Button(fr_rotavap_fn, text = "Add", width=5, height=1, command = lambda:rotaVap_stopRot()).grid(row=3, column= 3, padx=3, pady=3)
tk.Button(fr_rotavap_fn, text = "Add", width=5, height=1, command = lambda:rotaVap_stopVac()).grid(row=4, column=3, padx=3, pady=3)
tk.Button(fr_rotavap_fn, text = "Add", width=5, height=1, command = lambda:rotaVap_lift(ent_rotavap_mm)).grid(row=5, column=5, padx=3, pady=3)
tk.Button(fr_rotavap_fn, text = "Start", width=5, height=1, command = lambda:rotaVap_Global(True, ent_rotavap_gtime)).grid(row=6, column=5, padx=3, pady=3)
tk.Button(fr_rotavap_fn, text = "Stop", width=5, height=1, command = lambda:rotaVap_Global(False, ent_rotavap_gtime)).grid(row=6, column=8, padx=3, pady=3)
tk.Button(fr_rotavap_fn, text = "Add", width=5, height=1, command = lambda:rotaVap_heating(ent_rotavap_temp)).grid(row=7, column=5, padx=3, pady=3)

#realy Panel

fr_relay = tk.Frame(window, width =450, height = 700,relief = tk.FLAT, bd =2)
fr_relay.grid(row=0, column=1, padx=5,pady=5)
fr_relay.grid_propagate(0)
fr_relay.rowconfigure(4, weight=1)
fr_relay.columnconfigure(0, weight=1)

#relay Initialization

tk.Label(fr_relay, text = "Relay Initialization Command Editor", width=50, height=2, font=("calibri",12,"bold")).grid(row=0, column=0, padx=3, pady=3)

fr_relay_init = tk.Frame(fr_relay, width=65, height = 370, relief=tk.GROOVE, bd=2)
fr_relay_init.grid(row=1, column=0, padx=3, pady=3, sticky="nsew")

for i in range(0,6):

    for j in [0,3]:

        tk.Label(fr_relay_init, text = "Relay %d"%(i+1+j*2), width=10, height=2).grid(row=i, column=j, padx=3, pady=3)    

        globals()["com_relay_%d_init"%(i+1+j*2)] = ttk.Combobox(fr_relay_init, width=8, value = com_value)
       
        globals()["com_relay_%d_init"%(i+1+j*2)].grid(row=i, column=j+1, padx=3, pady=3)
       
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",1,com_relay_1_init.get())).grid(row=0, column=2, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",2,com_relay_2_init.get())).grid(row=1, column=2, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",3,com_relay_3_init.get())).grid(row=2, column=2, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",4,com_relay_4_init.get())).grid(row=3, column=2, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",5,com_relay_5_init.get())).grid(row=4, column=2, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",6,com_relay_6_init.get())).grid(row=5, column=2, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",7,com_relay_7_init.get())).grid(row=0, column=5, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",8,com_relay_8_init.get())).grid(row=1, column=5, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",9,com_relay_9_init.get())).grid(row=2, column=5, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",10,com_relay_10_init.get())).grid(row=3, column=5, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",11,com_relay_11_init.get())).grid(row=4, column=5, padx=3, pady=3)
tk.Button(fr_relay_init, text = "Add", width=5, height=1, command = lambda:instru_init("relay",12,com_relay_12_init.get())).grid(row=5, column=5, padx=3, pady=3)


#Relay Number

fr_relay_num = tk.Frame(fr_relay, width=65, height = 100, relief=tk.FLAT, bd=2)
fr_relay_num.grid(row=2, column=0, padx=3, pady=3, sticky="nsew")

tk.Label(fr_relay_num, text = "\tCommand Editor for Relay ", width=40, height=2, font=("calibri",12,"bold")).grid(row=0, column=0, padx=3, pady=3)

com_relay = []
for i in range (1,13):
    com_relay.append(i)

com_relay_num = ttk.Combobox(fr_relay_num, width=4, height=6, font=("calibri",12,"bold"), value = com_relay)
com_relay_num.grid(row=0, column=1, padx=3, pady=3)


#Relay Function

fr_relay_fn = tk.Frame(fr_relay, width=65, height = 190, relief=tk.GROOVE, bd=2)
fr_relay_fn.grid(row=3, column=0, padx=3, pady=3, sticky="nsew")
fr_relay_fn.rowconfigure(5, minsize=10, weight=1)
fr_relay_fn.columnconfigure(6, minsize=10, weight=1)
fr_relay_fn.grid_propagate(0)


tk.Label(fr_relay_fn, text = "After ", width=20, height=2).grid(row=0, column=0, padx=3,pady=3)

ent_relay_time = tk.StringVar()
       
tk.Entry(fr_relay_fn, textvariable = ent_relay_time, width=10).grid(row=0, column=1, padx=3, pady=3)

tk.Label(fr_relay_fn, text = "min ", width=8, height=2).grid(row=0, column=2, padx=3,pady=3)

tk.Label(fr_relay_fn, text = "Go to position ", width=20, height=2).grid(row=1, column=0, padx=3,pady=3)

ent_relay_position = tk.StringVar()
       
tk.Entry(fr_relay_fn, textvariable = ent_relay_position, width=10).grid(row=1, column=1, padx=3, pady=3)

tk.Button(fr_relay_fn, text = "Add", width=5, height=1, command = lambda:relay_going(int(com_relay_num.get()), ent_relay_time, ent_relay_position)).grid(row=1, column=2, padx=3, pady=3)
...
tk.Button(fr_instru, text = "RotaVap",width=10, height=1,command = lambda:topWin(fr_rotavap)).grid(row =8, column = 0, padx=3, pady=3)
tk.Button(fr_instru, text = "Relay",width=10, height=1,command = lambda:topWin(fr_relay)).grid(row =9, column = 0, padx=3, pady=3)


def createNewWindow():
    newWindow = tk.Toplevel(fr_instru)
    newWindow.title("Database Window")
    newWindow.geometry("1250x500+150+80")
    # newWindow.rowconfigure(0, weight=1)
    # newWindow.columnconfigure(2, weight=1)
    # newWindow.resizable(0, 0)

    # https://github.com/dmnfarrell/pandastable/issues/144

    df = pd.read_csv('Database_tmp.csv', index_col=False)
    newDf = pd.DataFrame(df,
                         columns=['Experiment', 'Reaction class', 'Substrates', 'Agents', 'Product', 'Scale', 'Yiele',
                                  'Duration', 'Status'])

    table = Table(newWindow, model=TableModel(dataframe=newDf),
    ...
    table.doExport(f"Database_{tp}.csv")

buttonExample = tk.Button(fr_instru, text="Database", width=15, height=2, font=("calibri", 12, "bold"),
                          command=createNewWindow)
buttonExample.grid(row=10, column=0, padx=3, pady=3)
