"""
Automated synthesis script for Esonarimod.
...
def initialize_milliGAT():
	...
	# "A": pump channel identifier
	# 0.005: execution delay (min)
	# 10000.00: flow rate (µL/min)
...
def rotaVap_drying():
	# dry the product using rotary evaporation to obtain the solid
...
def reaction_1():
    ...
    This function performs the full sequence of operations for the first reaction step
    """
	...
	first_step_liquid_liquid_separation() # Liquid-liquid separation
	rotaVao_20min_200to75mbar() #Rotary evaporation
	rotaVap_colletion() # Collect and concentrate the product
	first_step_precipitation() # Precipitation
	rotaVap_colletion() # Collect and concentrate the product
	rotaVap_drying() # Drying product
def reaction_2():
	"""
    Execute reaction 2 workflow.
    This function performs the second reaction step
    """
	second_step_purge_argon() # Remove oxygen/moisture using argon purge
	second_step_preparation() # Prepare reagents for step 2
	second_step_reaction() # Perform reaction step 2
def reaction_1_and_2():
	"""
    Execute reaction 1 and reaction 2 in parallel.
    This function runs both workflows concurrently using multithreading, enabling parallel processing of independent reaction steps to improve throughput.
    """
...
def autoprog_start():
	"""
	Execute the semi-continuous automated multistep synthesis workflow for Esonarimod.
    """
	global rotaVap_1
	global valve_1
	global valve_2
